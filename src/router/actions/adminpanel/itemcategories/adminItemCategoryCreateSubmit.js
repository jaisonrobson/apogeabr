...
const action = async ({ request }) => {
    ...
    try {
        ...
        const initialRequestResponse = await axios.post(`${process.env.REACT_APP_BACKEND_HOST}/item_categories`, initialRequestPayload, {
            headers: {
        ...
        const itemCategoryId = initialRequestResponse.data.id

        const requestGroups = groupData(filterEntries(allValues, "DIFY_"), "DIFY")

        const translationRequests = _.map(requestGroups, (value) => {
            const requestPayload = { item_category_translation: value }

            return axios.post(`${process.env.REACT_APP_BACKEND_HOST}/item_categories/${itemCategoryId}/translations`, requestPayload, {
                headers: {
                    'Content-Type': 'multipart/form-data',
                    'Authorization': `Bearer ${localStorage.getItem('token')}`,
                }
        ...
        window.location.assign(
            `${ROUTES.USER_ADMIN_PANEL_LIBRARYANDMAP_ITEMCATEGORIES.path.slice(0, -1)}?success=${encodeURIComponent(JSON.stringify(successMessages))}`
        )
    } catch (error) {
