...
const action = async ({ request }) => {
    ...
    const presentationId = form.get('id')
    ...
    try {
        ...
        if (!_.isEmpty(initialRequestValues)) {
            const initialRequestPayload = { presentation: initialRequestValues }

            const initialRequestResponse = await axios.put(`${process.env.REACT_APP_BACKEND_HOST}/presentations/${presentationId}`, initialRequestPayload, {
                headers: {
        ...
        if (!_.isEmpty(onlyTouchedTranslationFields)) {
            _.forEach(onlyTouchedTranslationFields, async (value) => {
                const requestPayload = { presentation_translation: _.omit(value, "isPersistedField") }

                const translationRequest = await axios.request({
                    url: value.isPersistedField
                        ? `${process.env.REACT_APP_BACKEND_HOST}/presentations/${presentationId}/translations/${value.locale_id}`
                        : `${process.env.REACT_APP_BACKEND_HOST}/presentations/${presentationId}/translations`,
        ...
        window.location.assign(
            `${ROUTES.USER_ADMIN_PANEL_PRESENTATIONS.path.slice(0, -1)}?success=${encodeURIComponent(
        ...
        return redirect(`${ROUTES.USER_ADMIN_PANEL_PRESENTATIONS.path.slice(0, -1)}?errors=${encodeURIComponent(JSON.stringify(resultingError))}`)
    }
}
