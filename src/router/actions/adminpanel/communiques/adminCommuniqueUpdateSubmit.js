...
const action = async ({ request }) => {
    ...
    const communiqueId = form.get('id')
    ...
    try {
        ...
        if (!_.isEmpty(initialRequestValues)) {
            const initialRequestPayload = { communique: initialRequestValues }

            const initialRequestResponse = await axios.put(`${process.env.REACT_APP_BACKEND_HOST}/communiques/${communiqueId}`, initialRequestPayload, {
                headers: {
        ...
        if (!_.isEmpty(onlyTouchedTranslationFields)) {
            _.forEach(onlyTouchedTranslationFields, async (value) => {
                const requestPayload = { communique_translation: _.omit(value, "isPersistedField") }

                const translationRequest = await axios.request({
                    url: value.isPersistedField
                        ? `${process.env.REACT_APP_BACKEND_HOST}/communiques/${communiqueId}/translations/${value.locale_id}`
                        : `${process.env.REACT_APP_BACKEND_HOST}/communiques/${communiqueId}/translations`,
        ...
        window.location.assign(
            `${ROUTES.USER_ADMIN_PANEL_COMMUNIQUES.path.slice(0, -1)}?success=${encodeURIComponent(
        ...
        return redirect(`${ROUTES.USER_ADMIN_PANEL_COMMUNIQUES.path.slice(0, -1)}?errors=${encodeURIComponent(JSON.stringify(resultingError))}`)
    }
}
