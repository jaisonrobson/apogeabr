...
const action = async ({ request }) => {
    ...
    try {
        ...
        if (!_.isEmpty(initialRequestValues)) {
            const initialRequestPayload = { nature: initialRequestValues }

            const initialRequestResponse = await axios.put(`${[process.env.REACT_APP_BACKEND_HOST]}/natures/${natureId}`, initialRequestPayload, {
                headers: {
        ...
        if (!_.isEmpty(onlyTouchedFieldsRequests)) {
            _.forEach(onlyTouchedFieldsRequests, async (value) => {
                const requestPayload = { nature_translation: _.omit(value, "isPersistedField") }

                const translationRequest = await axios.request({
                    url: value.isPersistedField
                        ? `${process.env.REACT_APP_BACKEND_HOST}/natures/${natureId}/translations/${value.locale_id}`
                        : `${process.env.REACT_APP_BACKEND_HOST}/natures/${natureId}/translations`,
        ...
        window.location.assign(
            `${ROUTES.USER_ADMIN_PANEL_LIBRARYANDMAP_NATURES.path.slice(0, -1)}?success=${encodeURIComponent(
        ...
        return redirect(`${ROUTES.USER_ADMIN_PANEL_LIBRARYANDMAP_NATURES.path.slice(0, -1)}?errors=${encodeURIComponent(JSON.stringify(resultingError))}`)
    }
}
