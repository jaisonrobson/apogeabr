...
const action = async ({ request }) => {
    ...
    const validationData = _.reduce(initialRequestValues, (acc, value, key) => {
        if (key.startsWith('validate-')) {
            const imageId = key.replace('validate-', '')
            acc[imageId] = Boolean(Number(value)) // Converte 'on' para boolean
        }
        return acc
    }, {})
    ...
    try {
        ...
        const initialRequestResponse = await axios.post(`${process.env.REACT_APP_BACKEND_HOST}/images/validate`, initialRequestPayload, {
            headers: {
        ...
        window.location.assign(
            `${ROUTES.USER_ADMIN_PANEL_IMAGES.path.slice(0, -1)}?success=${encodeURIComponent(JSON.stringify(successMessages))}`
        )
    } catch (error) {
        const resultingError = error?.response?.data || { message: error.message }

        return redirect(`${ROUTES.USER_ADMIN_PANEL_IMAGES.path.slice(0, -1)}?errors=${encodeURIComponent(JSON.stringify(resultingError))}`)
    }
}
