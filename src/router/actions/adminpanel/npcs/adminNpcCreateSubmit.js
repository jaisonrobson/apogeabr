...
const action = async ({ request }) => {
    ...
    try {
        ...
        const initialRequestResponse = await axios.post(`${process.env.REACT_APP_BACKEND_HOST}/npcs`, initialRequestPayload, {
            headers: {
        ...
        const npcId = initialRequestResponse.data.id

        const requestGroups = groupData(filterEntries(allValues, "DIFY_"), "DIFY")

        const translationRequests = _.map(requestGroups, (value) => {
            const requestPayload = { npc_translation: value }

            return axios.post(`${process.env.REACT_APP_BACKEND_HOST}/npcs/${npcId}/translations`, requestPayload, {
                headers: {
                    'Content-Type': 'multipart/form-data',
                    'Authorization': `Bearer ${localStorage.getItem('token')}`,
                }
        ...
        window.location.assign(
            `${ROUTES.USER_ADMIN_PANEL_LIBRARYANDMAP_NPCS.path.slice(0, -1)}?success=${encodeURIComponent(JSON.stringify(successMessages))}`
        )
    } catch (error) {
